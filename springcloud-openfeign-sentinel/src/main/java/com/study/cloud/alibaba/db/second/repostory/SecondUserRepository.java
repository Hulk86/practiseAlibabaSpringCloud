package com.study.cloud.alibaba.db.second.repostory;
 ...
 */
@Repository
public interface SecondUserRepository extends JpaRepository<SecondUserEntity,String>{

    Optional<SecondUserEntity> findByName(String name);

    Optional<SecondUserEntity> findBySerialNum(String id);

    Optional<SecondUserEntity> findByBindAccountId(String bindAccount);
   ...
   // @Modifying
    ...
    @Query(value = "select * from users as a where a.bind_account_id in (select b.account from account as b where b.role like  %:userRole% and b.status='normal')"
            , nativeQuery = true)
    List<SecondUserEntity>  findAllSimpleUserListByRole(@Param("userRole") String userRole);


  @Query(value = "SELECT * FROM users WHERE bind_account_id is null or LENGTH(bind_account_id) =0 order by id desc;"
      , nativeQuery = true)
  List<SecondUserEntity>  findAllUnbindUserList();
