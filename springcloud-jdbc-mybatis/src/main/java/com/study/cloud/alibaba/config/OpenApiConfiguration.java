...
public class OpenApiConfiguration implements WebMvcConfigurer {
  ...
  @Bean
  public Docket api3() {
    Docket docket = new Docket(DocumentationType.SWAGGER_2).groupName("mybatisplus")
        .forCodeGeneration(true)
        .useDefaultResponseMessages(false)
        .genericModelSubstitutes(ResponseEntity.class)
        .select()
        .apis(RequestHandlerSelectors.basePackage("com.study.cloud.alibaba.mybatisplus.db.controller"))
        .paths(PathSelectors.any())
        .build();
    docket.apiInfo(apiInfo());
    return docket;
